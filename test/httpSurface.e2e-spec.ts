...
import { ESystemKeyPrefix } from "../src/modules/systemKeyModule/enum/ESystemKeyPrefix.enum";
import { SystemApiKeyEntity } from "../src/modules/systemKeyModule/systemApiKey.entity";
...
describe("HTTP surface", () => {
  ...
  beforeAll(async () => {
    ...
    await ds.manager.save(
      SystemApiKeyEntity,
      Object.assign(new SystemApiKeyEntity(), {
    ...
    systemKey = credential.credential;
  });
  ...
  afterAll(async () => {
    await ds.manager.delete(SystemApiKeyEntity, { name: "e2e" });
    await app.close();
  });

  const auth = () => ({ Authorization: `Bearer ${systemKey}` });

  async function importDoc(body: unknown = doc()) {
  ...
  describe("system key auth", () => {
    it.each([
      ["/v1/projects", "get"],
      ["/v1/internal/actors/validate", "post"],
    ])("rejects %s without a key", async (path, method) => {
      await request(app.getHttpServer())
        [method as "get" | "post"](path)
        .expect(401);
    });

    it("rejects a wrong secret for a real key id", async () => {
      const keyId = systemKey.split(".")[0];
      await request(app.getHttpServer())
        .get("/v1/projects")
        .set({ Authorization: `Bearer ${keyId}.wrongsecret` })
        .expect(401);
    });

    it("returns the same body whatever the reason", async () => {
      const missing = await request(app.getHttpServer())
        .get("/v1/projects")
        .expect(401);
      const wrong = await request(app.getHttpServer())
        .get("/v1/projects")
        .set({ Authorization: "Bearer nlg_system_aaaaaaaaaaaa.nope" })
        .expect(401);
